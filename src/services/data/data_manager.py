...
REQUIRED_COLUMNS = [
    'run_id', 'run_name', 'run_state', 'created_at',
    'config.optim', 'config.top_k', 'config.top_p',
    'config.adam_beta1', 'config.adam_beta2', 'config.hidden_act',
    'config.max_length', 'config.min_length', 'config.model_type',
    'config.vocab_size', 'config.hidden_size', 'config.temperature',
    'config.hub_model_id', 'config.rms_norm_eps', 'config.weight_decay',
    'config.architectures', 'config.learning_rate', 'config.max_seq_length',
    'config.num_train_epochs', 'config.initializer_range',
    'config.intermediate_size', 'config.lr_scheduler_type',
    'config.num_hidden_layers', 'config.num_attention_heads',
    'config.num_key_value_heads', 'config.model.num_parameters',
    'config.max_position_embeddings', 'config.tokenizer_name',
    'config.attn_implementation', 'config.head_dim', 'config.dataset_name',
] + list(METRIC_MAPPING.values())  # Add all metrics to required columns
...
class HallucinationDataManager:
    ...
    def get_model_metrics(self, model_type: str) -> Dict[str, Any]:
        ...
        try:
            model_data = self.df[self.df['config.model_type'] == model_type]
            if len(model_data) == 0:
                return {}

            metrics = {}
            for friendly_name, column_name in METRIC_MAPPING.items():
                if column_name in model_data.columns:
                    values = model_data[column_name].dropna()
                    if len(values) > 0:
    ...
    async def _fetch_wandb_data(self) -> pd.DataFrame:
        """Fetch data from W&B using the proven parsing approach"""
        api = wandb.Api()
            ...
            for key, value in metrics.items():
                ...
                if 'value' in value:
                    ...
                    for subkey, subvalue in value.items():
                        metric_name = f"{prefix}{key}.{subkey}" if prefix else f"{key}.{subkey}"
                        if isinstance(subvalue, dict):
                            flattened.update(flatten_metrics({subkey: subvalue}, f"{prefix}{key}."))
                        else:
                    ...
                    if key == 'eval/accuracy':
                        try:
                            if isinstance(value, str):
                                ...
                                parsed = value
                            if isinstance(parsed, dict):
                                metrics['eval.accuracy.true_count'] = parsed.get('true_count')
                                metrics['eval.accuracy.true_fraction'] = parsed.get('true_fraction')
                                logger.info(f"Extracted accuracy for run {run.id}: {parsed}")
                            continue
                        except Exception as e:
                            logger.error(f"Error parsing accuracy for run {run.id}: {str(e)}")
                            metrics[key.replace('/', '.')] = value
                    else:
                        parsed = parse_nested_value(value)
                        if isinstance(parsed, dict) and 'value' not in parsed:
                            metrics.update(flatten_metrics(parsed, f'{key.replace("/", ".")}.'))
                        else:
        ...
        if 'eval.accuracy.true_fraction' in df.columns:
            ...
            logger.info(f"Number of accuracy values: {df['eval.accuracy.true_fraction'].count()}")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Loaded DataFrame with {len(self.df)} rows and {len(columns_to_keep)} columns")
    ...
    async def _init_caches(self):
        ...
        try:
            ...
            cache_data = {
                'model_types': self.df['config.model_type'].unique().tolist() if 'config.model_type' in self.df.columns else [],
                'learning_rates': sorted(self.df['config.learning_rate'].dropna().unique().tolist()) if 'config.learning_rate' in self.df.columns else [],
    ...
    def _compute_metrics_summary(self) -> Dict[str, Any]:
        ...
        metrics = ['eval.accuracy.true_fraction', 'eval.f1', 'eval.precision', 'eval.recall']
        summary = {}
    ...
    def get_best_models(self, metric: str = 'accuracy', top_k: int = 5) -> List[Dict]:
        ...
        try:
            ...
            if metric_col not in self.df.columns:
                logger.warning(f"Metric {metric_col} not found in data. Available metrics: {[col for col in self.df.columns if col.startswith('eval.') or col.startswith('train.')]}")
                return []
            
            columns = ['run_id', 'config.model_type', 'config.learning_rate', metric_col]
            available_cols = [col for col in columns if col in self.df.columns]
            
            # Get best models
            best_models = self.df.nlargest(top_k, metric_col)[available_cols].to_dict('records')
    ...
    def compare_hyperparams(self, metric: str = 'eval.accuracy.true_fraction') -> Dict[str, Any]:
        """Compare hyperparameter performance"""
        try:
            comparisons = {}
            
            if 'config.learning_rate' in self.df.columns:
                lr_stats = self.df.groupby('config.learning_rate')[metric].agg([
                    'mean', 'std', 'count'
                ]).round(4).to_dict('index')
                comparisons['learning_rate'] = lr_stats
            
            if 'config.num_train_epochs' in self.df.columns:
                epoch_stats = self.df.groupby('config.num_train_epochs')[metric].agg([
                    'mean', 'std', 'count'
                ]).round(4).to_dict('index')
